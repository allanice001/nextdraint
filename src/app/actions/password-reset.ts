...
export async function resetPassword(formData: FormData) {
	try {
		...
		if (!validatedFields.success) {
			return { success: false, message: "Invalid input. Please check your password." }
		}

		// Validate the token and get the associated email
		const email = await import("@/lib/password-reset").then((mod) => mod.validatePasswordResetToken(token))

		if (!email) {
			return { success: false, message: "Invalid or expired token. Please request a new password reset link." }
		}
		...
		await import("@/lib/password-reset").then((mod) => mod.deletePasswordResetToken(token))
